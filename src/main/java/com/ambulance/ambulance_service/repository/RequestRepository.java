...
public interface RequestRepository extends JpaRepository<Request, Long> {
    ...
    /**
     * Find all non-deleted requests with pagination
     * @param pageable pagination information
     * @return Page of non-deleted requests
     */
    Page<Request> findByDeletedFalse(Pageable pageable);

    /**
     * Find all requests ordered by request time in descending order (newest first)
     * @return List of requests sorted by request time in descending order
     */
    @Query("SELECT r FROM Request r ORDER BY r.requestTime DESC")
    ...
    @Query("SELECT r FROM Request r WHERE r.status = :status ORDER BY r.requestTime ASC")
    List<Request> findByStatusOrderByRequestTimeAsc(@Param("status") RequestStatus status);
}